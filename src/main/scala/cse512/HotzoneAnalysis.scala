...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    joinDf.createOrReplaceTempView("joinRes");
//======================================
	val result = joinDf.groupBy("rectangle").count().orderBy(asc("rectangle"));
        result.show()
    //joinDf.show();

    //val output = spark.sql("select rectangle, count(rectangle) from joinRes group by rectangle order by rectangle ASC")
    //res.show();
    //val outputdf = output.toDF();
    //outputdf.show();
//output.createOrReplaceTempView("joinRes");    
//res.createOrReplaceTempView("joinResult")
